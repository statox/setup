#!/usr/bin/env bash
#
# Immich ML orchestrator - scale-to-zero controller for the Immich ML ECS task.
#
# This script is meant to run periodically with cron
#
#   required env vars set - IMMICH_URL, IMMICH_API_KEY, AWS_REGION,
#   ECS_CLUSTER, ECS_SERVICE, ROUTE53_ZONE_ID, ML_DNS_NAME, STATE_FILE,
#   IDLE_GRACE_SECONDS, MAX_SCALED_UP_SECONDS. Example:
#     */2 * * * * IMMICH_URL=... IMMICH_API_KEY=... ... run.sh
#
# Can be run manually:
#   set -a && . .env && set +a && test.sh
#
# - Check Immich's pending ML job queue
#   - This part is buggy because Immich queue's don't retain failed tasks.
#     If the ml-container is down and a task is triggered and fails then
#     immich removes it from the queue. Confirm with some Github issues.
#   - Instead we should be more systematic: always start the container
#     and always restart the queues. Or find other calls to the API
#     to check the queues candidates.
# - Use `decide_action` to update the desired count of the fargate service
#   - Scale up: also update the DNS with the new task IP + Retrigger ML jobs in a failed/skipped state
#       - The retrigger is broken since queues don't really stay in failed/skipped state they 
#         just clean up failed tasks.
#   - Scale down:
#       - After IDLE_GRACE_SECONDS of no pending jobs
#       - Forced if the service has been up for longer than MAX_SCALED_UP_SECONDS regardless of queue state.
#
# How: state (idle/scaled-up timestamps) is persisted between ticks in STATE_FILE
