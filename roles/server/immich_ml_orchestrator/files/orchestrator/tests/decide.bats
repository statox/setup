...
@test "decide_action: failed work present, service down -> scale_up" {
    run decide_action 3 0 "" 1000 600
    [ "$status" -eq 0 ]
    [ "$output" = "scale_up" ]
}

@test "decide_action: failed work present, service already up -> noop" {
    run decide_action 3 1 "" 1000 600
    [ "$output" = "noop" ]
}

@test "decide_action: no failed work, service down -> noop" {
    run decide_action 0 0 "" 1000 600
    [ "$output" = "noop" ]
}

@test "decide_action: no failed work, service up, no idle timer yet -> wait_idle" {
    run decide_action 0 1 "" 1000 600
    [ "$output" = "wait_idle" ]
}

@test "decide_action: no failed work, service up, idle timer running, grace not elapsed -> wait_idle" {
    run decide_action 0 1 900 1000 600
    [ "$output" = "wait_idle" ]
}

@test "decide_action: no failed work, service up, idle grace exactly elapsed -> scale_down" {
    run decide_action 0 1 400 1000 600
    [ "$output" = "scale_down" ]
}

@test "decide_action: no failed work, service up, idle grace long elapsed -> scale_down" {
