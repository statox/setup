...
This module doesn't work completely because of how immich handles its queues

For now the process is to
- Update the Desired Count in the AWS console https://eu-west-3.console.aws.amazon.com/ecs/v2/clusters/immich-ml/services/immich-machine-learning/logs?region=eu-west-3#
- Run update-dns.sh on panda so that immich can find the new task
- The task may take a few minutes to start
- Reset the queues manually in immich's UI
- The task is killed when there is no more activity
